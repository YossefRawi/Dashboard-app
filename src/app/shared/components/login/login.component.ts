...
export class LoginComponent implements OnInit {
  ...
  logFormData() {
    ...
    if (!this.adminForm.valid) {
      this.error = 'Form is Invalid! please enter a valid Email.';
      return;
    }
    this.http
      .get(url)
      ...
      .then((data: any) => {
        ...
        if (isAdmin) {
          alert(
            `Success: ${this.adminForm.value.email} has Admin permissions.`
          );
          this.globalStateService.setIsLogged(true);
          this.router.navigate(['/home']);
          return;
        } else {
          return (this.error = `You aren't authorized. Only and ADMIN is authorized.`);
        }
      })
